...
<td id="tdLista" valign="top">
 ...
 <div > 
  <% if(fechaCorrecta){ %>
		...
		<%= bundle.getString("confCancelarTurno") +" " %> <%= doctor %> <%= bundle.getString("confSacarTurno2")+" " %> <%= fecha %> <%= bundle.getString("confSacarTurno3")+" " %> <%= hora %>
		</p>
		
		
		<form method="POST" action="ConfirmadoCancelar.do">
	...
	<%}else{ %>
		...
		<%= bundle.getString("fechaInvalidaCan") %>
		</p>
		<%}%>
  </div>
  </td>
